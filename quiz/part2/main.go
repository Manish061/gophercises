...
func main() {
	...
	limit := flag.Int64("limit", 20, "an Integer representing time in seconds")
	...
	givenAnswer := ""
	score := 0
	quit := time.After(time.Duration(*limit) * (time.Second))
...
loop:
	for i := 0; i < len(quiz.Question); i++ {
		...
		go func() {
			fmt.Scanf("%s\n", &givenAnswer)
			quizChan <- givenAnswer
		}()
		select {
		case <-quit:
			fmt.Println()
			break loop
		case givenAnswer := <-quizChan:
			if givenAnswer != "" {
